import "../App.css";
import { MenuItem, Switch, Select, TextField, Paper } from "@material-ui/core";
import "react-dropdown/style.css";
import { useRootStore, RootStoreProvider } from "../stores/RootStore";
import { observer } from "mobx-react";
import AudioEngine from "../assets/AudioEngine.tsx";
import { useState } from "react";

const ControlPanel = () => {
  let { instruments, settings } = useRootStore();
  const [tempo, setTempo] = useState(60);
  const [useBioTempo, setUseBioTempo] = useState(false);
        ...
        <div className="instrumentControlPanel">
          {Object.keys(instruments).map((key) => (
            <div
              ...
              className={`control-item control${key + 1}`}
            >
              <Select
                className="instrumentSelect"
                ...
                onChange={(e) => {
                  instruments[key].audio = e.target.value;
                }}
              >
                <MenuItem value="">
                  <em>Instrument</em>
                ...
                {Object.keys(settings.preset).map((key) => (
                  <MenuItem key={key} value={settings.preset[key].filePath}>
                    {settings.preset[key].name}
                  </MenuItem>
                ))}
              </Select>
              <TextField
                ...
                label="Total Notes"
                type="number"
                aria-label={`Select total notes for track number ${
                  parseInt(key) + 1
                }`}
                title={`Select total notes for track number ${
                  parseInt(key) + 1
                }`}
              ...
              <TextField
                ...
                label="On Notes"
                type="number"
                aria-label={`Select on notes for track number ${
                  parseInt(key) + 1
                }`}
              ...
              <Switch
                ...
                aria-label={`${
                  instruments[key].getDisplay() === true ? "Mute" : "Unmute"
                } track number ${parseInt(key) + 1}`}
                title={`${
                  instruments[key].getDisplay() === true ? "Mute" : "Unmute"
                } track number ${parseInt(key) + 1}`}
                checked={instruments[key].getDisplay() === true ? true : false}
                onChange={() => {
                  instruments[key].toggle();
                }}
              />
            </div>
          ))}
